...
from mani_skill.agents.robots import SO100, Fetch, Panda, WidowXAI, XArm6Robotiq, OpenArm
...
class PickCubeEnv(BaseEnv):
    ...
    agent: Union[Panda, Fetch, XArm6Robotiq, SO100, WidowXAI, OpenArm]
    cube_half_size = 0.02
    goal_thresh = 0.025
    cube_spawn_half_size = 0.05
    cube_spawn_center = (0, 0)
        
    def __init__(self, *args, robot_uids="openarm", robot_init_qpos_noise=0.02, **kwargs):
    ...
    def _load_agent(self, options: dict):
        if self.robot_uids != "openarm":
            super()._load_agent(options, sapien.Pose(p=[-0.615, 0, 0]))
        else:
            rotation = euler2quat(0, 0, np.pi)
            new_pose = sapien.Pose(p=[-0.3, 0, 0], q=rotation)
            super()._load_agent(options, new_pose)
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        with torch.device(self.device):
            ...
            if self.agent.keyframes:
                key = list(self.agent.keyframes.keys())[0]
                self.agent.robot.set_qpos(
                    self.agent.keyframes[key].qpos
                )
